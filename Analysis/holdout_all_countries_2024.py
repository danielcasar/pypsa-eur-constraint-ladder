"""
All-22-country 1-year-ahead holdout, pooled-FE capacity-share model.

Train: 2015-2023.  Forecast: 2024 deviation profile, all countries.
Compare to observed 2024 deviation profile per country.

Reuses the pipeline from predict_deviation_pooled_capacity.py:
  - capacity-share panel
  - pooled OLS with country FE + universal tech slopes
  - LOO-residual bootstrap PI

Outputs:
  Analysis/results/holdout_all_countries_2024.csv
  Analysis/results/holdout_all_countries_2024_summary.txt
  Analysis/results/plots/holdout_all_countries_2024_rmse.{pdf,png}
  Analysis/results/plots/holdout_all_countries_2024_panels.{pdf,png}
"""
...
print(f"Panel: {len(panel)} rows total; train ({len(panel_train)} rows, "
      f"years <= {TRAIN_END}); target year {TARGET}")


# ---------------------------------------------------------------------------
# Fit per-hour pooled OLS with country FE
# ---------------------------------------------------------------------------
...
print(f"  Hours fit: {len(m)}/24")

# LOO across years on train data only -> calibrated PI bootstrap source
...
print(f"  LOO residual pool: {len(loo)} rows across {len(loo_resids_per_hour)} hours")


# ---------------------------------------------------------------------------
# 2024 forecast for every country
# ---------------------------------------------------------------------------
print(f"\nForecasting {TARGET} for {len(COUNTRIES)} countries ...")
rows = []
for cc in COUNTRIES:
    de_train = (panel_train[panel_train["country"] == cc]
                [["year"] + SHARES].drop_duplicates())
    if len(de_train) < 2:
        print(f"  {cc}: insufficient train data, skip")
        continue
    extr = extrapolate_capshare(de_train, TARGET)
    obs_country = panel[(panel["country"] == cc) &
                        (panel["year"] == TARGET)]
    for h in range(24):
...
fc.to_csv(RESULTS / "holdout_all_countries_2024.csv", index=False)
...
def metrics_per_country(d: pd.DataFrame) -> dict:
    ...
    if not m.any():
        return {"n_hours": 0, "rmse": np.nan, "mae": np.nan,
                "cov90": np.nan, "cov50": np.nan, "obs_range": np.nan,
                "obs_std": np.nan}
...
for cc, g in fc.groupby("country"):
    r = metrics_per_country(g.sort_values("hour"))
    met_rows.append({"country": cc, **r})
met = pd.DataFrame(met_rows).sort_values("rmse")


# ---------------------------------------------------------------------------
# Plot 1: per-country RMSE bar chart, sorted
# ---------------------------------------------------------------------------
...
ax.axhline(m_plot["rmse"].median(), color="#d62728",
           linewidth=0.8, linestyle="--",
           label=f"median = {m_plot['rmse'].median():.1f} €/MWh")
ax.set_xticks(range(len(m_plot)))
ax.set_xticklabels(m_plot["country"], rotation=0)
...
fig.savefig(PLOTS / "holdout_all_countries_2024_rmse.pdf", dpi=300, bbox_inches="tight")
fig.savefig(PLOTS / "holdout_all_countries_2024_rmse.png", dpi=300, bbox_inches="tight")
plt.close(fig)


# ---------------------------------------------------------------------------
# Plot 2: small multiples — every country's 2024 holdout fan vs observed
# ---------------------------------------------------------------------------
...
fig.suptitle(f"1y-ahead holdout per country (train 2015-{TRAIN_END}, target {TARGET}, 90% PI)",
             fontsize=8, y=1.005)
fig.savefig(PLOTS / "holdout_all_countries_2024_panels.pdf", dpi=300, bbox_inches="tight")
fig.savefig(PLOTS / "holdout_all_countries_2024_panels.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
out.append(f"Train years: 2015-{TRAIN_END}.  Target year: {TARGET}.")
out.append("=" * 78)
out.append(f"\nMedian RMSE across {len(met.dropna(subset=['rmse']))} countries: "
           f"{met['rmse'].median():.2f} €/MWh")
out.append(f"Mean   RMSE: {met['rmse'].mean():.2f} €/MWh")
out.append(f"Min / Max:   {met['rmse'].min():.2f}  /  {met['rmse'].max():.2f} €/MWh")
out.append(f"Median 90% PI coverage: {met['cov90'].median()*100:.0f}%  (target 90%)")
out.append(f"Median 50% PI coverage: {met['cov50'].median()*100:.0f}%  (target 50%)")
out.append("")
...
print()
print(text)
(RESULTS / "holdout_all_countries_2024_summary.txt").write_text(text, encoding="utf-8")
print(f"\nWrote: {RESULTS / 'holdout_all_countries_2024_summary.txt'}")
